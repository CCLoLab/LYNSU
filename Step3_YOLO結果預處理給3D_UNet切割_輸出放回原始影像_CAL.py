...
def process_brain_region(seg_target, num_of_yolo_box, x1, x2, y1, y2, whole_brain,max_dim=1023):
    ...
    for box_num in range(1, num_of_yolo_box + 1):
        ...
        print('==========調查用==========')
        print('原始YOLO_X_length: ',X_length)
        print('原始YOLO_Y_length: ',Y_length)
        # 最終發現: 垂直方向Y的偵測框需要直接擴大，無須再拓展成正方形的區塊拓展，直接在原始特徵框就拓展。
#         Y_start = max(Y_start - 30, 0)
        # 只對底端特徵框加大範圍是因為: MB特徵框頂部絕對高於CAL，只有底部可能會裁切到CAL
        Y_end = min(Y_end + 50, max_dim)
        X_length, Y_length = X_end - X_start, Y_end - Y_start
        print('增加長度後_X_length: ',X_length)
        print('增加長度後_Y_length: ',Y_length)
        print('==========調查結束==========')
